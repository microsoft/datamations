  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    shiny::observeEvent(pipeline(), {
      ...
      if (pipeline_group_by) {
        ...
        grouping_vars <- dplyr::group_vars(state_2)
        grouping_vars <- rlang::parse_exprs(grouping_vars)

        data_states_tabs[[2]] <- state_2 %>%
          dplyr::ungroup() %>%
          dplyr::select(!!!grouping_vars, rlang::parse_expr(inputs$summary_var())) %>%
        ...
        data_states_tabs[[3]] <- data_states[[3]] %>%
          dplyr::ungroup() %>%
          dplyr::select(!!!grouping_vars, rlang::parse_expr(inputs$summary_function())) %>%
          dplyr::arrange(!!!grouping_vars)
