...
function prep_specs_summarize(states, groupby, summarize, output) {
  ...
  if (groupby.length > 2) {
    ...
    domain = [
      _.round(Math.min(output[groups[0]] - _error[groups[0]], output[groups[1]] - _error[groups[1]]), 13),
      _.round(Math.max(output[groups[0]] + _error[groups[0]], output[groups[1]] + _error[groups[1]]), 13),
    ];
  }
