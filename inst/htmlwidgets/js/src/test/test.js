      ...
      if (key === "data") {
        for (var j = 0; j < specs[i][key].values.length; j++) {
          for (const field of Object.keys(specs[i][key].values[j])) {
            if (
              field.startsWith("datamations_y") ||
              field === "Lower" ||
              field === "Upper"
            ) {
              chai
        ...
        if (key === "layer") {
          for (j = 0; j < specs[i][key].length; j++) {
            chai
              ...
              .to.be.approximately(
                raw_spec[i][key][j].encoding.y.scale.domain[0],
                epsilon
              );
            chai
              .expect(specs[i][key][j].encoding.y.scale.domain[1])
              .to.be.approximately(
                raw_spec[i][key][j].encoding.y.scale.domain[1],
                epsilon
              );
          ...
          chai
            .expect(specs[i][key])
            .to.deep.equal(raw_spec[i][key], "failed layer#" + j);
          ...
          for (j = 0; j < specs[i][key].layer.length; j++) {
            chai
              ...
              .to.be.approximately(
                raw_spec[i][key].layer[j].encoding.y.scale.domain[0],
                epsilon
              );
            chai
              .expect(specs[i][key].layer[j].encoding.y.scale.domain[1])
              .to.be.approximately(
                raw_spec[i][key].layer[j].encoding.y.scale.domain[1],
                epsilon
              );
          ...
          chai
            .expect(specs[i][key])
            .to.deep.equal(raw_spec[i][key], "failed spec#" + i);
        } else {
          if (key === "encoding" && specs[i][key].y.scale) {
            ...
            chai
              ...
              .to.be.approximately(
                raw_spec[i][key].encoding.y.scale.domain[0],
                epsilon
              );
            chai
              .expect(specs[i][key].encoding.y.scale.domain[1])
              .to.be.approximately(
                raw_spec[i][key].encoding.y.scale.domain[1],
                epsilon
              );

            specs[i][key].encoding.y.scale.domain =
              raw_spec[i][key].encoding.y.scale.domain;
          }
          chai
            .expect(specs[i][key])
            .to.deep.equal(
              raw_spec[i][key],
              "failed spec#" + i + ", key=" + key
            );
        }
      }
    }
  ...
  describe("#generateGrid()", function () {
    ...
    it("should have datamations_x, datamations_y, gemini_id", function () {
      const invalid = gridValues.filter((d) => {
        return (
          d.datamations_x === undefined ||
          d.datamations_x === null ||
          d.datamations_y === undefined ||
          d.datamations_y === null ||
          d.gemini_id === undefined ||
          d.gemini_id === null
        );
      });
  ...
  before(function (done) {
    fs.readFile(
      ...
      function (err, fileContents) {
        ...
        fs.readFile(
          "../../../../sandbox/products_mean_rating.json",
          "utf8",
          function (err, fileContents) {
            if (err) throw err;
  ...
  before(function (done) {
    fs.readFile(
      ...
      function (err, fileContents) {
        ...
        fs.readFile(
          "../../../../sandbox/penguins_three_groups.json",
          "utf8",
          function (err, fileContents) {
            if (err) throw err;
            penguins = JSON.parse(fileContents);
            fs.readFile(
              "../../../../sandbox/penguins_median_specs.json",
              "utf8",
              function (err, fileContents) {
                if (err) throw err;
                                        ...
                                        fs.readFile(
                                          ...
                                          function (err, fileContents) {
                                            ...
                                            fs.readFile(
                                              ...
                                              function (err, fileContents) {
                                                if (err) throw err;
                                                sum_specs_two_columns =
                                                  JSON.parse(fileContents);
                                                fs.readFile(
                                                  "../../../../inst/specs/prod_specs.json",
                                                  "utf8",
                                                  function (err, fileContents) {
                                                    if (err) throw err;
                                                    prod_specs =
    ...
    context("group by single column and summarize by product", function () {
      it("should match", function () {
        const specs = datamations.specs(
          ...
          "Product of Salary",
          {
            Masters: 5.89224682818428e140,
            PhD: 2.94265906927814e54,
          }
        );
    ...
    context("group by two columns and summarize by product", function () {
      it("should match", function () {
        const specs = datamations.specs(
          ...
          {
            Masters: {
              Academia: 17535325577809800000,
              Industry: 3.36021524210573e121,
            },
            PhD: {
              Academia: 6.02776193570217e34,
              Industry: 48818435443657800000,
            },
          }
        );
