...
When ggplot2 code is provided, the order of animation is not determined by the order in `group_by()`, but by the plot layout. Variables are first animated by what's in the column facets, then the row facets, the x-axis, and finally by color. If color is the same as one of the other grouping variables (e.g. the same as the x-axis variable), it's animated in the same step. 
