...
#'   datamations:::parse_pipeline()
parse_pipeline <- function(pipeline, supported_tidy_functions = c("group_by", "summarize", "filter")) {
  pipeline %>%
...
#' @noRd
split_pipeline <- function(pipeline, supported_tidy_functions = c("group_by", "summarize", "filter")) {
  pipeline <- pipeline %>%
    stringr::str_split("%>%") %>%
    purrr::pluck(1) %>%
    stringr::str_trim()
...
#' @noRd
parse_data_from_first_function <- function(pipeline, supported_tidy_functions = c("group_by", "summarize", "filter")) {
